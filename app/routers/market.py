...
async def ws_status(
    ...
    """WebSocket connection status and live price cache stats."""
    from app.services.angel_ws import ws_stats
    return ws_stats()


@router.post("/ws/prices")
async def ws_prices(
    payload: LTPRequest,
    _: User = Depends(require_master),
):
    """
    Get live prices from WebSocket in-memory cache.
    Falls back to REST if WebSocket not connected.
    """
    from app.services.angel_ws import get_live_prices, is_ws_connected
    from app.core.database import SessionLocal

    if is_ws_connected():
        prices = get_live_prices(payload.security_ids)
        return {
            "source":  "websocket",
            "fetched": len(prices),
            "total":   len(payload.security_ids),
            "data":    {sid: {"ltp": ltp, "prev_close": None} for sid, ltp in prices.items()},
        }
