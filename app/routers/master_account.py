...
async def test_master_account(
    ...
    # CRITICAL: update in-memory cache so algo engine uses new token
    # Without this, the old cached token stays and Dhan rejects it (each login
    # invalidates the previous token)
    if result["success"] and result.get("access_token"):
        from app.services.master_token import _cached_token, _cached_client_id, _cache_fetched_at
        import app.services.master_token as mt
        import time
        mt._cached_token     = result["access_token"]
        mt._cached_client_id = decrypt(acc.dhan_client_id)
        mt._cache_fetched_at = time.monotonic()
