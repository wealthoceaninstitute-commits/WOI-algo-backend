...
Built directly from https://dhanhq.co/docs/v2/

Covers:
  - Order Book        GET  /v2/orders
  - Order by ID       GET  /v2/orders/{order-id}
  ...
  - Modify Order      PUT  /v2/orders/{order-id}
  - Cancel Order      DELETE /v2/orders/{order-id}
"""
...
def _client(access_token: str, proxy_host=None, proxy_port=443, proxy_user=None, proxy_pass=None):
    headers = {
...
async def get_order_by_id(access_token: str, order_id: str, **proxy) -> dict:
    async with _client(access_token, **proxy) as c:
        r = await c.get(f"{API_BASE}/orders/{order_id}")
        if r.status_code == 200:
            return {"success": True, "data": _ok(r)}
        return {"success": False, "message": f"Dhan {r.status_code}: {r.text[:300]}"}
...
# ── Trade Book ────────────────────────────────────────────────────────────────
# GET /v2/trades — all executed trades for the day
...
async def get_trades_by_order(access_token: str, order_id: str, **proxy) -> dict:
    async with _client(access_token, **proxy) as c:
        r = await c.get(f"{API_BASE}/trades/{order_id}")
        if r.status_code == 200:
            return {"success": True, "data": _ok(r)}
        return {"success": False, "message": f"Dhan {r.status_code}: {r.text[:300]}"}
...
async def get_holdings(access_token: str, **proxy) -> dict:
    async with _client(access_token, **proxy) as c:
        r = await c.get(f"{API_BASE}/holdings")
        if r.status_code == 200:
            return {"success": True, "data": _ok(r)}
        return {"success": False, "message": f"Dhan {r.status_code}: {r.text[:300]}"}
...
async def get_fund_limit(access_token: str, **proxy) -> dict:
    async with _client(access_token, **proxy) as c:
        r = await c.get(f"{API_BASE}/fundlimit")
        if r.status_code == 200:
            data = _ok(r)
            return {
                "success": True,
...
# ── Place Order ───────────────────────────────────────────────────────────────
# POST /v2/orders
# Requires static IP whitelisting on Dhan

async def place_order(
    access_token: str,
    dhan_client_id: str,
    transaction_type: str,          # BUY | SELL
    exchange_segment: str,          # NSE_FNO | BSE_FNO | NSE_EQ etc
...
def normalize_position(p: dict) -> dict:
    """Convert Dhan position response fields to our internal format."""
    ...
    pos_type    = p.get("positionType", "")          # LONG | SHORT | CLOSED
...
def normalize_trade(t: dict) -> dict:
    """Convert Dhan trade response to our format."""
    return {
