...
from app.services.market_data import fetch_ltp, fetch_first_candle
...
async def _fetch_and_save_prev_close(
    token: str, client_id: str, universe: list[dict], db: Session
) -> int:
...
async def _fresh_master_token() -> tuple[str, str]:
    """
    Return master data token — uses master_token service which:
    - Caches in memory for 23h (no repeated Dhan logins)
    - Falls back to DB if cache empty
    - Only re-authenticates if genuinely expired
    Never use DhanCredential (client token) here — data is master-only.
    """
    db = SessionLocal()
    try:
...
async def _keep_token_alive_until(
    ...
    while True:
        ...
        try:
            prices = await fetch_ltp(token, client_id, HEARTBEAT_SID)
            if prices:
...
async def run_daily_algo():
    ...
    try:
        ...
        token, master_client_id = await _fresh_master_token()
        ...
        if existing < len(combined_universe) * 0.9:
            saved = await _fetch_and_save_prev_close(token, master_client_id, combined_universe, db)
            for _, (run, _) in runs.items():
        ...
        # Dhan market data tokens expire after ~30 min of inactivity
        await _keep_token_alive_until(9, 12, 30, token, master_client_id)
        ...
        open_count = await _fetch_and_save_open_price(token, master_client_id, combined_universe, db)
