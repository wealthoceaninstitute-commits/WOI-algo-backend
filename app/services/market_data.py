...
async def get_master_token(db: Session) -> tuple[str, str]:
    """
    DEPRECATED: Use app.services.master_token.get_master_token instead.
    Kept for backward compatibility only.
    Returns (access_token, client_id) for master data account.
    """
    acc = _get_account(db)
    if not acc:
        raise RuntimeError(
...
async def fetch_ltp(
    ...
    """
    # Token fingerprint — first 12 + last 6 chars so we can track
    # which token is being used without exposing the full JWT
    t_fp = f"{token[:12]}...{token[-6:]}" if token and len(token) > 18 else "EMPTY"
    print(f"[market_data] fetch_ltp: client={client_id} "
          f"token={t_fp} stocks={len(security_ids)}")
