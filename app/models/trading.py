...
class DhanCredential(Base):
    ...
    dhan_client_id = Column(String, nullable=False)          # Dhan client ID (encrypted)
    pin = Column(String, nullable=False)                     # Encrypted
    totp_secret = Column(String, nullable=False)             # Encrypted
    access_token = Column(Text, nullable=False)              # Encrypted (long JWT)

    is_active = Column(Boolean, default=False)
    last_verified = Column(DateTime(timezone=True), nullable=True)
    token_expires_at = Column(DateTime(timezone=True), nullable=True)
    last_error = Column(String, nullable=True)               # Last connection error message
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
